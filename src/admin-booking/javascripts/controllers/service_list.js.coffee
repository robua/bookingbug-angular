...
angular.module('BB.Directives').directive 'bbAdminServices', () ->
  restrict: 'AE'
  replace: true
  scope : true
  controller : 'AdminServiceList'

angular.module('BB.Controllers').controller 'AdminServiceList',($scope, $rootScope, $q, $attrs, $modal, $sce, ItemService, FormDataStoreService, ValidatorService, PageControllerService, halClient, AlertService, ErrorService, $filter, CategoryService, AdminServiceService) ->

  $scope.controller = "public.controllers.AdminServiceList"

  FormDataStoreService.init 'AdminServiceList', $scope, [
    'service'
  ]
  ...
  $scope.init = (comp) ->
    ...
    ppromise = AdminServiceService.query($scope.bb)
    @skipped = false
    ppromise.then (items) =>
